      ...
      try {
        ...
        const mediastackRes = await fetch(
          `https://api.mediastack.com/v1/news?access_key=${
            import.meta.env.VITE_MEDIASTACK_KEY
          }&keywords=artificial+intelligence&sort=published_desc&limit=10`
        );
        if (!mediastackRes.ok) {
          throw new Error(
        ...
        const gnewsRes = await fetch(
          `https://gnews.io/api/v4/search?q=AI&lang=en&max=10&apikey=${
        ...
        const newsDataRes = await fetch(
          `https://newsdata.io/api/1/news?apikey=${
            import.meta.env.VITE_NEWSDATA_KEY
          }&q=AI&language=en`
        );
        if (!newsDataRes.ok) {
          throw new Error(
